...
open class TabBarButton: UIControl {
    ...
    open override var isSelected: Bool {
        didSet {
            ...
            updateTintForSelectedState(isSelected)
        }
    }
...
extension TabBarButton {
    ...
    private func updateTintForSelectedState(_ state: Bool) {
        guard let tintColor = barButtonItem.style.tintColor else { return }
        if state == true {
            contentView.imageView?.tintColor = tintColor.selected
        } else {
            contentView.imageView?.tintColor = tintColor.normal
        }
    }
}
